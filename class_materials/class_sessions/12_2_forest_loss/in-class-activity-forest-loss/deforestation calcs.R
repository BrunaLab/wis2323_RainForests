...
peru <- read_csv(here("class_materials", "class_sessions", "/12_2_forest_loss", "in-class-activity-forest-loss","Primary Forest loss in Peru","treecover_extent_2000_in_primary_forests_2001_tropics_only__ha.csv"),
) %>% select(-area__ha) %>% rename (cover=umd_tree_cover_extent_2000__ha) %>% mutate(year=2000)
rwanda <- read_csv(here("class_materials", "class_sessions", "/12_2_forest_loss", "in-class-activity-forest-loss","Primary Forest loss in Rwanda","treecover_extent_2000_in_primary_forests_2001_tropics_only__ha.csv"),
)%>% select(-area__ha) %>% rename (cover=umd_tree_cover_extent_2000__ha) %>% mutate(year=2000)

cover_2000<-bind_rows(peru,rwanda)

peru_loss <- read_csv(here("class_materials", "class_sessions", "/12_2_forest_loss", "in-class-activity-forest-loss","Primary Forest loss in Peru","treecover_loss_in_primary_forests_2001_tropics_only__ha.csv"),
) %>% select(iso, year=umd_tree_cover_loss__year, loss=umd_tree_cover_loss__ha)
rwanda_loss <- read_csv(here("class_materials", "class_sessions", "/12_2_forest_loss", "in-class-activity-forest-loss","Primary Forest loss in Rwanda","treecover_loss_in_primary_forests_2001_tropics_only__ha.csv"),
)%>% select(iso, year=umd_tree_cover_loss__year, loss=umd_tree_cover_loss__ha)
